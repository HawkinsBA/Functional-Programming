...
op '+' = eatSpace *> (Add <$ char '+')
op '*' = eatSpace *> (Mul <$ char '*')
...
parseExp con add mul = (foldExpr <$>) . evalParser (parseExpr <* eof)
  where
    foldExpr (Lit x) = con x
    foldExpr (Add x y) = add (foldExpr x) (foldExpr y)
    foldExpr (Mul x y) = mul (foldExpr x) (foldExpr y)
